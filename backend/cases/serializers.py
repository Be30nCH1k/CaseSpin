...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        # создаю пользователя и сразу выдаю токены
    ...
    def to_representation(self, instance):
        # добавляю токены в ответ после регистрации
...
class UpgradeSkinSerializer(serializers.ModelSerializer):
    weapon_name = serializers.CharField(
        source='item.weapon_name'
    )

    skin_name = serializers.CharField(
        source='item.skin_name'
    )
...
class TargetItemSerializer(serializers.Serializer):
    ...
    chance_percentage = serializers.FloatField(
        required=False,
        help_text='Динамический шанс относительно выбранного предмета'
    )


class UpgradePerformSerializer(serializers.Serializer):
    item_id = serializers.IntegerField(
        help_text='ID предмета из инвентаря для апгрейда'
    )
    ...
    target_item_id = serializers.IntegerField(
        help_text='ID целевого предмета'
    )
...
class UpgradeResultSerializer(serializers.Serializer):
    ...
    won_item = serializers.DictField(
        required=False
    )
    ...
    chance_used = serializers.FloatField(
        required=False
    )
...
class ContractPerformSerializer(serializers.Serializer):
    item_ids = serializers.ListField(
        ...
        min_length=3,
        max_length=10,
        help_text='ID предметов из InventoryItem от 3 до 10 штук'
    )
